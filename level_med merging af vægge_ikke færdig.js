...
export default class Level{
	...
	arr_to_floor(scene,grid_floor,floor_number){
		...
		let walls = [];
		let wallGeometry = new THREE.BufferGeometry().fromGeometry(new THREE.BoxGeometry(1,1,1));
		...
		for (let row = 0; row < array_temp.length; row++){
			...
			for (let column = 0; column < array_temp[row].length; column++){
				...
				case "1":
					...
					geometry.translate(column*this.TILESIZE + 0.5*this.TILESIZE*wall_width,
						floor_number*3.5,
						row*this.TILESIZE + 0.5*this.TILESIZE * wall_height);
					...
					geometry.position.x = column*this.TILESIZE + 0.5*this.TILESIZE*wall_width; 
					geometry.position.y = floor_number*3.5;
					geometry.position.z = row*this.TILESIZE + 0.5*this.TILESIZE * wall_height;
					*/
					
					
					geometry.scale(wall_width * this.TILESIZE,this.TILESIZE*5,wall_height * this.TILESIZE);
		...
		let geometryWalls = BufferGeometryUtils.mergeBufferGeometries(walls);
		
		let materialWalls = new THREE.MeshStandardMaterial({ map: wall});
		let meshWalls = new THREE.Mesh(geometryWalls,materialWalls);
		scene.add(meshWalls);
