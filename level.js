			...
			for (let column = 0; column < array_temp[row].length; column++){
				...
				switch(array_temp[row][column]){
					...
					case "s": // mangler indretning, skal vaere uden loft
						makeRoom.Make_Room(roomStartPosition, scene,0,'Room_s',0,'trappe',' ')
						break;
					case "t": // Mangler modeller, skal vaere uden loft
						makeRoom.Make_Room(roomStartPosition, scene,0,'Room_t',0,'trappe\n2',' ')
						break;
					...
					case "G": // MANGLER OPDATERING TIL INDRETNING, Skal være uden gulv
						makeRoom.Make_Room(roomStartPosition, scene,0,'Room_G1',1,'trappe',' ')
						break;
					case "H":
